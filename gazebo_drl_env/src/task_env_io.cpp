...
RL::ParamLoad::ParamLoad(ros::NodeHandlePtr rosNode_pr_): rosNodeConstPtr(rosNode_pr_)
{
    ...
    assert(rosNodeConstPtr->getParam("/HEIGHT_LIMITATION", heightLimitation));
...
logFileName("/home/zoe/catkin_ws/src/gazebo_drl_env/log/logger.txt")
{
    // generate robot name
...
{
    std::vector<float> allAgentPosition;
    geometry_msgs::Twist actionOut;
    ...
    // get all agents' position to avoid collision
    for (int i = 0;i < this->AGENT_NUM;i++)
    {
        ignition::math::Pose3d robotState = RL::GazePose2IgnPose(FindPosebyName(ROBOT_NAMES[i]));
        allAgentPosition.push_back(robotState.Pos().X());
        allAgentPosition.push_back(robotState.Pos().Y());
    }
    ...
    for (int i = 0;i < this->AGENT_NUM;i++)
    {
        if (ROBOT_NAMES[i] == robotName)
        {
            ...
            if (this->mapType == 1)
                positionGenerator->Mode_1_Agent_8_FourBisection_DiagonalTarget(i, allAgentPosition);
            else if (this->mapType == 3)
                positionGenerator->Mode_3_Agent_16_NoTarget(i, allAgentPosition);
            else if (this->mapType == 4)
                positionGenerator->Mode_4_Agent_1_RandomTarget(i, allAgentPosition);
            else if (this->mapType == 5)
                positionGenerator->Mode_5_Closer_Agent_16_NoTarget(i, allAgentPosition);
            else if (this->mapType == 6)
                positionGenerator->Mode_6_Agent_2_OppositeTarget(i, allAgentPosition);
            else if (this->mapType == 7)
...
bool RL::TaskEnvIO::EdgeCheck(const ignition::math::Pose3d robot_pose_) const
{
    ...
    // check wrong states
    if (robot_pose_.Pos()[2] > paramList->heightLimitation)
        return true;
